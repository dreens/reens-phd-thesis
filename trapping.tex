...
\subsection{Ring and Tricycle Spectroscopy}
...
If it were necessary to instead use microwaves to transfer molecules directly from a trapped to an untrapped state by flipping their magnetic quantum number, $|f, \frac{3}{2}\rangle$ to $|f, -\frac{3}{2}\rangle$, scanning the trap would require scanning the applied microwave frequency between $1.7$ and $10$~GHz.
...
In particular, the microwaves are delivered to the trap using a bias tee setup, nicely described in~\citep[Fig.~5]{stuhl2012uwave}, and the transfer function through the isolation capacitors is particularly troublesome.
Once an effort to measure the transfer function using a network analyzer was performed, and the transfer function was found to vary in the required frequency range by over $40$~dB.
In contrast, with the entire population setting below $0.5$~T, only $13$~MHz needs to be scanned out of $1.7$~GHz, and no significant attenuation variation is expected or measured.
...
Inherent in this procedure are several key assumptions:
\begin{enumerate}
\item Microwaves are equally efficient at transferring molecules at all locations.
\item Microwaves are equally efficient at transferring molecules of all velocity classes.
\item Laser induced fluorescence is equally efficient at all locations.
\item The population is not significantly perturbed by the spectroscopy.
\end{enumerate}
These assumptions vary in their validity.
For example, the influence on microwave frequency dependent intensity has already been claimed negligible above, but nonetheless microwave intensity varies significantly across the cloud.
This goes against typical arguments pertaining to the wavelength of the microwave frequency (some $20$~cm) because the microwaves are in the near field regime with respect to the conductors that form the trapping potential.
Because of this, microwave fields may to a good approximation be taken to be dominantly electric and equal to the DC electric field distribution generated during loading but with direction oscillating at the microwave frequency.
...
Points clearly fall below the fit at high magnetic fields, and also seem to lie below at the lowest fields as well, although the latter effect is clearly much less significant.
