...
An example of the simulated trajectories arising from a specific electric field is also shown in fig.~\ref{fig:eil}.
...
\includegraphics[width=8cm]{ohz1}\\
\includegraphics[width=8cm]{ohz3}
\includegraphics[width=8cm]{ohz9}
...
\figdave{decay2state}{Simulated Electric Field Loss with Secondary Substates}{Some Caption}{14cm}
...
\figdave{trikencol}{Mapping Molecule Number to Collisions}{Collision rates are simulated as a function of molecule number. With $1000$ molecules, a rate of $0.068$ /s/mol is found.}{14cm}
...
This similarity suggests that the two sequences are not significantly different from one another thermodynamically, and leaves one wondering whether any systematic artifact in the forwards - backwards comparison could be playing an additional role.
...
We can think about the implications of this observable by first reviewing the behavior of a correctly functioning evaporation sequence~\cite{Luiten1996}.
By maintaining a fixed ratio between the average energy of the trapped molecules and the depth of the trap, often parametrized as $\eta$ and fixed in the range of $5-15$, the situation can be contrived that each molecule which leaves the trap carries away a disproportionate fraction of the kinetic energy of the ensemble.
For molecules which are not 
